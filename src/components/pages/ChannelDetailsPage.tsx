...
export default function ChannelDetailsPage({
  ...
  const handleRetryAutoFix = async (log: AutoFixLog) => {
    ...
    try {
      setChecking(true);
      componentLogger.log(`[AutoFix] Retrying fix for ${channel.channelName}...`);
  ...
  const triggerAutoFix = async () => {
    ...
    try {
      setChecking(true);
      componentLogger.log(`[AutoFix] Starting automatic fix for ${channel.channelName}...`);
  ...
  // Fetch auto-fix logs when channel changes
  useEffect(() => {
    if (!mounted || !channel) return;
